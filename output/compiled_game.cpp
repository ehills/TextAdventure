			...
			#define WELCOME_MESSAGE "Your name is Andy an amateur ghost hunter. You have arrived at your first job. Infront of you is the old, haunted mansion locals say to be inhabited by a long dead sycthe weilding psycho. This is the perfect place to hone your ghost busting skills."
...
Location* entranceHall = new Location("The entrance hall", "You climb through the window and find yourself in a dimly lit hall illuminated only by the faint moonlight filtering through the dusty windows. You are sourrounded in darkness, but can make out doorways to the east and west.");
Location* garden = new Location("The overgrown garden", "You are in the overgrown garden in front of the mansion. The front door is bolted shut.");
...
Location* inventory = new Location("inventory", "Description of the inventory");Player* andy = new Player();andy->setLocation(garden);andy->setInventory(inventory);andy->setMaxItems(4);
...
garden->addItem("wrench", &wrench);
wrench.setLocation(garden);
wrench.addAttribute("canPickup", true);
cout << endl << "			" << GAME_NAME << endl;cout << CREDITS << endl << endl;cout << WELCOME_MESSAGE << endl << endl;prompt = "\n>>> ";cout << andy->getLocation()->printNameAndDescription() << endl;cout << andy->getLocation()->listItems() << endl;while (true) {   main_loop:   cout << prompt;   getline(cin, command);   istringstream word(command);   verb = "";   noun = "";   count = 0;   while (word) {      if (count > 2) {           cout << "I do not understand your command. Enter 2 words at most, a verb followed by a noun" << endl;           goto main_loop;      }      if (count == 0) {           word >> verb;      } else {           word >> noun;      }      count++;   }			if (command == QUIT_GAME) {
			quit_loop:
			string quit = "";
...
break;
} else {cout << andy->getLocation()->printNameAndDescription() << endl;
cout << andy->getLocation()->listItems() << endl;
}
...
goto main_loop;}
			...
			cout << andy->getInventory()->listItems() + " ";
...
cout << " A massive bed";
} else {cout << "You find a small key glinting under the bed.";
...
door.setAttribute("isLocked", false);
} else {cout << "You can't use the key here.";
}
goto main_loop;}if (verb == "pickup" || verb == "pick-up" || verb == "get") {
...
} else {garden->setNorth(entranceHall);
...
} else {cout << "You have already pried loose the window.";
}
} else {cout << "You do not have the wrench.";
}
} else {cout << "You cannot the wrench here.";
}
goto main_loop;}if (verb == "pickup" || verb == "pick-up" || verb == "get") {
