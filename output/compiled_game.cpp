...
if (verb == "use") {
if (lamp.getLocation() == andy->getInventory()) {
entranceHall->setDescription("With light eminating from the lamp you can now make out a door to the north as long with the doorways to the east and west.");entranceHall->setNorth(secretHall);
masterBedroom->setDescription("With the aid of the lamp you can make out a mirror in the far corner of the room.");masterBedroom->addItem(mirror.getName(), &mirror);
cout << "You fiddle around with the old lamp and manage to turn it on.";
} else {cout << "You don't have that item.";
}
goto main_loop;}
